...
func TestModify(t *testing.T) {
	...
	tests := []struct {
		...
		{&InfixExpression{Left: one(), Operator: "+", Right: two()}, &InfixExpression{Left: two(), Operator: "+", Right: two()}},
		{&InfixExpression{Left: two(), Operator: "+", Right: one()}, &InfixExpression{Left: two(), Operator: "+", Right: two()}},
		...
		{&ArrayLiteral{Elements: []Expression{one(), two()}}, &ArrayLiteral{Elements: []Expression{two(), two()}}},
	}
	...
	for keyT, valT := range hashLiteral.Pairs {
		key, _ := keyT.(*IntegerLiteral)
		if key.Value != 2 {
			t.Errorf("value is not %d. got=%d", 2, key.Value)
		}

		val, _ := valT.(*IntegerLiteral)
		if val.Value != 2 {
